...
/**
 * Animated Noise Overlay Component
 * 
 * Creates an animated film grain effect using a pre-generated set of 
 * noise frames to ensure high performance and zero browser lag.
 */
...
export default function AnimatedNoiseOverlay({ 
  loopDuration = 7.5,
  autoStart = true
  ...
  useEffect(() => {
    ...
    const generateFrames = () => {
      ...
      for (let f = 0; f < numFrames; f++) {
        ...
        for (let i = 0; i < data.length; i += 4) {
          ...
          const gray = Math.floor(Math.pow(normalized, 1 / contrast) * 255);
          data[i] = gray;
          data[i + 1] = gray;
          data[i + 2] = gray;
          data[i + 3] = 255;
    ...
    const updateCanvasSize = () => {
      ...
      if (perlinNoiseRef.current && framesRef.current.length === 0) {
        generateFrames();
      }
    };
    ...
    if (autoStart && framesRef.current.length > 0) {
      animationFrameRef.current = requestAnimationFrame(animate);
    }
