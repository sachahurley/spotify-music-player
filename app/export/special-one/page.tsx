...
/**
 * Special One Video Export Page
 * 
 * This page exports ONLY the Special One asset and animations as an MP4 video.
 * No UI elements are included - just the animated background.
 * 
 * Technical Specifications:
 * - Aspect Ratio: 9:16 (vertical/portrait)
 * - Resolution: 1080×1920px
 * - Duration: 7 seconds (configurable 3-8 seconds)
...
export default function SpecialOneExportPage() {
  ...
  /**
   * Load the Special One background image
   */
  ...
  /**
   * Render a single frame of the Special One animation
   * 
   * This replicates the two animations:
   * 1. Blur animation (7s blur in/out)
   * 2. Noise overlay (7s CSS animation)
   * 
   * Note: No circular pan - background is static with calc(50% + 50px) center positioning
   */
  const renderFrame = (
    ctx: CanvasRenderingContext2D,
    ...
    // This means: center horizontally with 50px offset to the right
    const bgX = (width / 2) - (imgWidth / 2) + 50;
    const bgY = (height / 2) - (imgHeight / 2);

    // 1. BLUR ANIMATION
    // Replicate the blur animation: 0 → blurAmount → 0 over 7 seconds
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      // Download the file
      const filename = `special-one-${duration}s-${Date.now()}.mp4`;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        <div>
          <h1 className="text-3xl font-bold mb-2">Special One Export</h1>
          <p className="text-gray-400">
            Export the Special One asset and animations as an MP4 video (no UI elements).
          </p>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg">
          ...
          <input
            ...
            step="0.1"
            value={duration}
            onChange={(e) => setDuration(Math.max(3, Math.min(8, parseFloat(e.target.value) || 7)))}
            disabled={isExporting}
            className="w-full px-4 py-2 bg-gray-700 rounded-lg text-white"
          />
          <p className="mt-2 text-sm text-gray-400">
            Recommended: 7 seconds (matches animation loop). Must be between 3-8 seconds.
          </p>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg text-sm">
          ...
          <ul className="space-y-2 text-gray-300">
            ...
            <li>• Animations: Blur in/out and noise overlay</li>
          </ul>
        </div>
